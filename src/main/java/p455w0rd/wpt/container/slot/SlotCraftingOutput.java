...
public class SlotCraftingOutput extends AppEngCraftingSlot {
	...
	public void doClick(final InventoryAction action, final EntityPlayer who) {
		...
		if (container instanceof ContainerWPT) {
			wirelessTerminal = ((ContainerWPT) container).getWirelessTerminal();
		}
		if (!wirelessTerminal.isEmpty()) {
			shiftCraftToNetwork = WPTUtils.getShiftCraftMode(wirelessTerminal);
		}
		...
		for (int x = 0; x < maxTimesToCraft; x++) {
			if (ia.simulateAdd(rs).isEmpty()) {
				...
				if (action == InventoryAction.CRAFT_SHIFT && container instanceof ContainerWPT && shiftCraftToNetwork) {
					final ItemStack tmpStack = craftItem(who, rs, inv, all);
					final IAEItemStack tmpAEStack = AEApi.instance().storage().getStorageChannel(IItemStorageChannel.class).createStack(tmpStack);
					if (tmpAEStack != null) {
						final IAEItemStack aeStack = inv.injectItems(tmpAEStack, Actionable.MODULATE, mySrc);
	...
	protected IRecipe findRecipe(final InventoryCrafting ic, final World world) {
	...
	@Override
	protected NonNullList<ItemStack> getRemainingItems(final InventoryCrafting ic, final World world) {
		if (container instanceof ContainerCraftingTerm) {
			final ContainerCraftingTerm containerTerminal = (ContainerCraftingTerm) container;
			final IRecipe recipe = containerTerminal.getCurrentRecipe();
